...
export const authProvider: AuthProvider = {
  login: async ({ email }) => {
    try {
      ...
      // dataprovider.custom is used to make a custon request of the GraphQL API
      // this will call dataProvider which will go throug the fetchWrapper function
      const { data } = await dataProvider.custom({
        ...
        meta: {
          variables: { email },
          // pass the email to see of the user exists and if so, return the accessToken
          rawQuery: `
  ...
  check: async () => {
    try {
      ...
      // this is to know if the user is authentication or not
      ...
      // for ant ither error, redirect to the login page
      return {
        authenticated: false,
        redirectTo: "/login",
      };
    ...
    try {
      ...
      const { data } = await dataProvider.custom<{ me: any }>({
        ...
        headers: accessToken
          ...
          : {},
        meta: {
          // get the user infromation such as name, email, etc.
          rawQuery: `
                    query Me {
                        me {
                            id,
